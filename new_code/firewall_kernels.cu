...
__global__ void process_pkt(char *input_buf, char *output_buf, int *len, int num_pkts, int buf_len, unsigned long *rules, int num_lines, unsigned int *nat_table, unsigned long *nat_set) {
  int tx = threadIdx.x;
  int pkt_start;
    ...
    if (!drop) {
      ...
      if (((src_addr & 0xffffff00) == 0x0a000000) && ((dst_addr & 0xffffff00) != 0x0a000000)) {
        ...
        //printf("natset1 is %lu\n", nat_set[index1]);
          ...
          compute_tcp_checksum(&input_buf[pkt_start+14], (unsigned short *)input_buf[pkt_start+ip_header_len], num_lines);
...
void run_firewall(char *input_buf, char *output_buf, int *len, int num_pkts, int buf_len, unsigned long *rules, int num_lines, unsigned int *nat_table, unsigned long *nat_set) {	
